...
public class Engine implements IPlainTextDocumentEngine {
	...
	public Engine(String pFilePath, String pInputType, String pAlias) {
		DocumentRawType docType = DocumentRawType.RAW;
	...
	public RuleSet registerInputRuleSetForPlainFiles(List<List<String>> inputSpec) {
		...
		RuleSetCreator ruleSetCreator = new RuleSetCreator(lineblocks, inputSpec, "inputRuleSet"); //lineblocks inputSpec
		this.inputRuleSet =  ruleSetCreator.createRuleSet();
		//System.out.println(ruleSetCreator.createRuleSet().toString());
	...
	public int exportPdf(String outputFileName) {
		if(this.lineblocks.size() == 0)
			loadFileAndCharacterizeBlocks();
//dika mou
		//for(LineBlock e:this.document.getLineblocks()) {
		//System.out.println(	e.getDetermineFormatStatus());
	//	System.out.println(e.getDetermineHeadingStatus());
		//}
		 if((this.lineblocks !=null) && (this.inputRuleSet!= null))
				  characterizeLineblocks(this.document, this.inputRuleSet);
...
//	public int yetAnotherMethod() { 
	  //    return  this.loadRawDocument(alias);
	...
	private void characterizeLineblocks(Document document, RuleSet ruleSet) {
		List<LineBlock> lineblocks=this.document.getLineblocks();
		
		Objects.requireNonNull(this.document);
		Objects.requireNonNull(lineblocks);
		Objects.requireNonNull(ruleSet);
		
		if(this.document.getInputFileType() == DocumentRawType.RAW)
			characterizeRawFile(lineblocks, ruleSet);
		else if(this.document.getInputFileType() == DocumentRawType.ANNOTATED)
...
//	public void yetAnotherMethod1() {
		//	this.characterizeLineblocks(document, inputRuleSet);
	//}

	private void characterizeRawFile(List<LineBlock> lineblocks, RuleSet ruleSet) {
		for(LineBlock l:this.lineblocks) {
			l.setStyle(ruleSet.determineHeadingStatus(l));
			//System.out.println(ruleSet.determineHeadingStatus(l));
			l.setFormat(ruleSet.determineFormatStatus(l));
		//System.out.println(ruleSet.determineFormatStatus(l));
	//	System.out.println(l.getDetermineFormatStatus()+"\n");
			//System.out.println(l.getDetermineHeadingStatus()+"\n");
